...
describe('Posts E2E', () => {
  ...
  test('login -> create post -> like -> cleanup orphans', async () => {
    ...
    // create a post
    const createRes = await request(app.getHttpServer()).post('/posts').set('Authorization', `Bearer ${token}`).send({ content: 'e2e-test-post' }).expect(201)
    const postId = createRes.body.id
    expect(postId).toBeTruthy()

    // like the post
    const likeRes = await request(app.getHttpServer()).post(`/posts/${postId}/like`).set('Authorization', `Bearer ${token}`).expect(201)
